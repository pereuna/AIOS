...
static char *word(char **p, char *dst, size_t cap) {
    ...
    size_t n=0; while (*s && !ws(*s)) { if (n+1>=cap) return NULL; dst[n++]=*s++; }
    dst[n]=0; *p=s; return dst;
}
static int emit1(asm1_program *p, size_t *n, unsigned char x) {
    if (*n>=sizeof(p->code)) return ASM1_TOO_LARGE;
...
static int rr(asm1_program *p,size_t *n,unsigned char op,unsigned d,unsigned s) {
    if (d>=12||s>=12) return ASM1_REGISTER;
...
static int memop(asm1_program *p,size_t *n,unsigned char op,unsigned r,unsigned i) {
    if (r>=10||i>=256) return ASM1_REGISTER;
...
static int compare_set(asm1_program *p,size_t *n,unsigned d,unsigned s,unsigned char cc) {
    int e=rr(p,n,0x89,10,d); if (e) return e;
    e=rr(p,n,0x39,s,10); if (e) return e;
    e=rr(p,n,0x31,d,d); if (e) return e;
    return setcc(p,n,cc,d);
}
static int divop(asm1_program *p,size_t *n,unsigned d,unsigned s,int rem) {
    ...
    int e=rr(p,n,0x89,10,d); if (e) return e;
    e=rr(p,n,0x89,11,s); if (e) return e;
    e=emit1(p,n,0x31); if (e) return e; emit1(p,n,0xd2); /* xor edx,edx */
    e=rr(p,n,0x89,0,10); if (e) return e;
    emit1(p,n,0x41); emit1(p,n,0xf7); emit1(p,n,0xe3); /* div r11 */
    return rr(p,n,0x89,rem?2:0,d);
}
static int shiftop(asm1_program *p,size_t *n,unsigned d,unsigned s,int right) {
    int e=rr(p,n,0x89,10,d); if (e) return e; /* scratch destination */
    e=rr(p,n,0x89,1,s); if (e) return e; /* CL = count */
    emit1(p,n,0x41); emit1(p,n,0xd3); emit1(p,n,(unsigned char)((right?0xe8:0xe0)|2));
    return rr(p,n,0x89,d,10);
}
...
int asm1_compile(const char *source, asm1_program *p) {
    ...
    while (*cur) {
        ...
        if (!strcmp(w,"asm1")||!strcmp(w,"end")) continue;
        if (!strcmp(w,"input")) {
            while ((t=word(&q,x,sizeof(x)))) { uint32_t v; if (p->input_count>=ASM1_MAX_INPUT||number(x,&v)) { p->error_line=line; return p->error=ASM1_NUMBER; } p->input[p->input_count++]=v; }
            continue;
        }
        if (na>=ASM1_MAX_INSTRUCTIONS) { p->error_line=line; return p->error=ASM1_TOO_LARGE; }
        ...
        if (op==99) { p->error_line=line; return p->error=ASM1_SYNTAX; }
        a[na]=(insn){.op=op,.line=line};
        if (op==O_LABEL||op==O_JMP) { if (!word(&q,x,sizeof(x))||!label_ok(x)) { p->error_line=line; return p->error=ASM1_LABEL; } memcpy(a[na].label,x,4); if (op==O_LABEL) { if (nl>=32||find_label(labels,nl,x)>=0) { p->error_line=line; return p->error=ASM1_DUPLICATE; } memcpy(labels[nl].name,x,4); labels[nl++].at=na; } }
        else if (op==O_EXIT) { if (!word(&q,x,sizeof(x))||regno(x)<0) { p->error_line=line; return p->error=ASM1_REGISTER; } a[na].a=(unsigned)regno(x); saw_exit=1; }
        else if (op==O_ST) { if (!word(&q,x,sizeof(x))||!word(&q,y,sizeof(y))||regno(y)<0||number(x,&a[na].imm)||a[na].imm>=256) { p->error_line=line; return p->error=ASM1_SYNTAX; } a[na].b=(unsigned)regno(y); }
        else if (op==O_LD) { if (!word(&q,x,sizeof(x))||!word(&q,y,sizeof(y))||regno(x)<0||number(y,&a[na].imm)||a[na].imm>=256) { p->error_line=line; return p->error=ASM1_SYNTAX; } a[na].a=(unsigned)regno(x); }
        else if (op==O_LI) { if (!word(&q,x,sizeof(x))||!word(&q,y,sizeof(y))||regno(x)<0||number(y,&a[na].imm)) { p->error_line=line; return p->error=ASM1_SYNTAX; } a[na].a=(unsigned)regno(x); }
        else if (op==O_JZ) { if (!word(&q,x,sizeof(x))||!word(&q,y,sizeof(y))||regno(x)<0||!label_ok(y)) { p->error_line=line; return p->error=ASM1_SYNTAX; } a[na].a=(unsigned)regno(x); memcpy(a[na].label,y,4); }
        else { if (!word(&q,x,sizeof(x))||!word(&q,y,sizeof(y))||regno(x)<0||regno(y)<0) { p->error_line=line; return p->error=ASM1_REGISTER; } a[na].a=(unsigned)regno(x); a[na].b=(unsigned)regno(y); }
        while (ws(*q)) q++;
        if (*q) { p->error_line=line; return p->error=ASM1_SYNTAX; }
        na++;
    }
    if (!saw_exit) return p->error=ASM1_NO_EXIT;
    size_t n=0; size_t at[ASM1_MAX_INSTRUCTIONS];
    if (mov_const(p,&n,0,(uint32_t)p->input_count)) { p->error=ASM1_TOO_LARGE; return p->error; }
    ...
    for (unsigned i=0;i<na;i++) {
        ...
        case O_EXIT:e=emit1(p,&n,0xcd)||emit1(p,&n,0x80);break;
        ...
        if (e) { p->error_line=in->line; return p->error=e; }
    }
